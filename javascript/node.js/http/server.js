...
const server = http.createServer((req, res) => {
  ...
  var rootPath = __dirname.substr(0, __dirname.length-4);
  fs.access(rootPath + req.url, fs.constants.R_OK, err => {
    //返回404
    if (err) {
      console.log(err);
      res.writeHead(404);
    ...
    //写入last-modified
    var lstModifiedTime = fs.statSync(rootPath + req.url).mtime;
    ...
    if (req.headers['if-modified-since'] &&
      ...
      fs.createReadStream(rootPath + req.url).pipe(res);
    }
